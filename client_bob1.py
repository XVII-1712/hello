...
def ldpc_decode(encoded_key, H):
    ...
    decoded_key = pyldpc.decode(H, encoded_key, snr=10, maxiter=1000)  # Increased maxiter for better convergence
    return decoded_key

def sift_keys(alice_measurements, bob_measurements, bob_bases, decoy_flags):
    return [a for a, b, base, decoy in zip(alice_measurements, bob_measurements, bob_bases, decoy_flags) if a == b and decoy == 0]
...
decoy_flags_data = client_socket.recv(1024).decode('utf-8')
