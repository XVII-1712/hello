...
def ldpc_error_correction(alice_key, bob_key):
    ...
    d_v, d_c = 3, 6  # Adjusted parameters
    n = find_valid_n(key_length, d_c)  # Ensure d_c divides n

    # Debugging prints
    print(f"Original key length: {key_length}, Adjusted length (n): {n}")
    ...
    # Debugging prints
    print(f"Adjusted Alice key bin: {alice_key_bin}")
    print(f"Adjusted Bob key bin: {bob_key_bin}")
    
    # Create LDPC code
    H, G = make_ldpc(n, d_v, d_c, systematic=True, sparse=True)
    
    # Debugging prints
    print(f"LDPC matrix H shape: {H.shape}")
    print(f"LDPC matrix G shape: {G.shape}")
    
    # Encode Alice's key
    encoded_alice_key = encode(G, alice_key_bin, snr=10)
    
    # Debugging prints
    print(f"Encoded Alice key: {encoded_alice_key.shape}")
    ...
    # Debugging prints
    print(f"Corrected Bob key: {corrected_bob_key.shape}")
...
plt.xlabel('Round')
plt.ylabel('QBER')
plt.title('Quantum Bit Error Rate over Rounds with Dynamic Conditions')
plt.legend()
plt.show()
