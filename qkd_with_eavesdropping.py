...
def eavesdrop(state, eavesdropping_level):
    if np.random.rand() < eavesdropping_level:
        # Eve intercepts and measures the state in a random basis
        eve_basis = np.random.choice(['Z', 'X'])
        outcome = measure_basis(state, eve_basis)
        # Eve resends the measured state
        if eve_basis == 'Z':
...
# Function to run multiple rounds of QKD with basis choice, noise, and eavesdropping
def run_qkd_rounds(rounds, noise_level, eavesdropping_level):
    alice_key = []
    bob_key = []
    alice_bases = []
    bob_bases = []
...
num_rounds = 100
noise_level = 0.3  # Noise level to simulate more challenging conditions
eavesdropping_level = 0.2  # Eavesdropping level to simulate the presence of an eavesdropper
final_key_length = 64  # Length of the final key after privacy amplification

# Run the QKD protocol and measure QBER
alice_key, bob_key, alice_bases, bob_bases, qber = run_qkd_rounds(num_rounds, noise_level, eavesdropping_level)
...
plt.xlabel('Round')
plt.ylabel('QBER')
plt.title('Quantum Bit Error Rate over Rounds with Eavesdropping')
plt.legend()
plt.show()
